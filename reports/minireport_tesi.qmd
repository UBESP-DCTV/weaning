...
daily |>
  ...
  #             values_fill = 0) |>
  ungroup() |>
  select(-id_univoco, -reintubato, -morto) |>
  tbl_summary( 
    by = sbt,
    statistic =  ~ "{median}  (range: {min}, {max})  (IQR: {p25}, {p75})",
    # digits =  ~ 2
...
diagram_links <- tribble(
  ~source,           ~target,           ~value,
  "all patients",    "RT success",      rt_yes,
  "all patients",    "RT failure",      rt_no,
  "RT success",      "SBT success",     sbt_yes,
  "RT success",      "SBT failure",     sbt_no,
  "SBT success",     "weaning success", wea_yes,
  "SBT success",     "weaning failure", wea_no
)

diagram_nodes <- tibble(
  name = c(as.character(diagram_links[["source"]]), 
